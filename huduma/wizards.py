...
class KaribuCenterCasesReportWizard(models.TransientModel):
    ...
    department_ids = fields.Many2many("huduma.department","department_report_rel","report_ids","department_ids", required=True, string="Departments")

    @api.multi
    def print_xlsx_report(self):
        domain = []
        if self.start_date:
            domain.append(('opened_date','>=',self.start_date))

        if self.end_date:
            domain.append(('opened_date','<=',self.end_date))

        department_ids = self.department_ids.mapped("id")
        if len(department_ids)>0:
            domain.append(('department_id','in',department_ids))

        cases = self.env['huduma.case.header'].search(domain).mapped('id')
